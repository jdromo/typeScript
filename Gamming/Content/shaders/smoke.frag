...
void main() {
    ...
    const vec3 c1 = vec3(.2);
    const vec3 c2 = vec3(.3);
    const vec3 c3 = vec3(.4);
    const vec3 c4 = vec3(.2);
    const vec3 c5 = vec3(.3);
    const vec3 c6 = vec3(.4);

    //This is how "packed" the smoke is in our area. Try changing 8.0 to 1.0, or something else
    vec2 p = gl_FragCoord.xy * 1.0 / resolution.xy;
